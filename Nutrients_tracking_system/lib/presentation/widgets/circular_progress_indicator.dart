...
class _CircularIndicatorPainter implements CustomPainter {
  ...
  void _drawStepArc(
    ...
    for (;
      ...
      if (!isSelectedColor) {
        _drawDotInPlaceOfUnselectedStep(
          canvas,
          rect,
          paint,
          stepAngle,
  ...
  void _drawDotInPlaceOfUnselectedStep(
    Canvas canvas,
    Rect rect,
    Paint paint,
    double stepAngle,
    double stepLength,
    double indexStepSize,
  ) {
    // Draw a dot in place of the unselected step
    // to avoid drawing the full arc
    final dotSize = indexStepSize / 3;
    final dotAngle = dotSize / (rect.width / 2);
    final dotRemove = removeRoundedCapExtraAngle;
    canvas.drawOval(
      Rect.fromCenter(
        center: Offset(
          rect.center.dx +
              math.cos(stepAngle + (dotRemove ? dotAngle : 0)) *
                  (rect.width / 2 - dotSize / 2),
          rect.center.dy +
              math.sin(stepAngle + (dotRemove ? dotAngle : 0)) *
                  (rect.width / 2 - dotSize / 2),
        ),
        height: dotSize,
        width: dotSize,
